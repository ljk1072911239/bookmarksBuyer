...
gerador = '''from sportsbetio.sportsbetio import SportsBetIO
from time import sleep

sportsbetio = SportsBetIO()
sportsbetio.liga('{liga}')
...
carregaDicionario = open(f'C:/Users/{login}/Desktop/beto/sportsbetio/ligasSbetsio.txt', 'r')
# Se o dicionário não existir, criaremos um novo
try:
    # Na variável dicionario, leremos o conteúdo do arquivo ligasSportsBet.txt, e transformaremos em um dicionário.
    ...
    open(f'C:/Users/{login}/Desktop/beto/sportsbetio/ligasSbetsio.txt', 'w').write('{}')
    dicionario = dict(eval(carregaDicionario.read()))
...
while True:
    ...
    salvaDicionario = open(f'C:/Users/{login}/Desktop/beto/sportsbetio/ligasSbetsio.txt', 'w')
    ...
    caminhoLiga = f'C:/Users/{login}/Desktop/beto/sportsbetio/{liga}'
    ...
    if not path.exists(caminhoLiga):
        ...
        with open(caminhoLiga+f'/{liga.lower()}SportsBetIO.py', 'w') as arquivo:
