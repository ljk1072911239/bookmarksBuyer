...
gerador = '''from sportingbet.sportingbet import SportingBet
from time import sleep

sportingbet = SportingBet()
sportingbet.liga('{liga}')                                                
while True:
    try:
        sportingbet.achaParticipante(
            localParticipante=
            'Insira aqui o caminho do CSS selector do primeiro time da pagina da liga,'             # <---
            'e nao esqueca de mudar as variaveis de contaPartida e contaParticipante.'              # <---
        )
        sportingbet.oddsPartida(
            localOdds=
            'Insira aqui o caminho do CSS selector da primeira odd de resultado da pagina da liga,' # <---
            'e nao esqueca de mudar as variáveis de contaPartida e contaParticipante.',             # <---
            localPontos=
            'Insira aqui o caminho do CSS selector da primeira pontuacao da pagina da liga,'        # <---
            'e nao esqueca de mudar as variaveis de contaPartida e situacao.'                       # <---
        )
...
carregaDicionario = open(f'C:/Users/{login}/Desktop/beto/sportingbet/ligasSportingBet.txt', 'r')
...
try:
    ...
    open(f'C:/Users/{login}/Desktop/beto/sportingbet/ligasSportingBet.txt', 'w').write('{}')
    dicionario = dict(eval(carregaDicionario.read()))
...
while True:
    ...
    salvaDicionario = open(f'C:/Users/{login}/Desktop/beto/ligasSportingBet', 'w')
    ...
    caminhoLiga = f'C:/Users/{login}/Desktop/beto/sportingbet/{liga}'
    ...
    if not path.exists(caminhoLiga):
        ...
        with open(caminhoLiga+f'/{liga.lower()}SportingBet.py', 'w') as arquivo:
